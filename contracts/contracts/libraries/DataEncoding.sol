...
} from "@matterlabs/zksync-contracts/contracts/l1-contracts/common/l2-helpers/L2ContractAddresses.sol";
import {
    LEGACY_ENCODING_VERSION,
    NEW_ENCODING_VERSION
} from "@matterlabs/zksync-contracts/contracts/l1-contracts/bridge/asset-router/IAssetRouterBase.sol";
import {
    AssetIdMismatch,
...
} from "@matterlabs/zksync-contracts/contracts/l1-contracts/common/L1ContractErrors.sol";
import {
    WrongMsgLength
} from "@matterlabs/zksync-contracts/contracts/l1-contracts/bridge/L1BridgeContractErrors.sol";
...
library DataEncoding {
    ...
    function decodeBridgeBurnData(
        ...
        internal
        pure
        returns (uint256 amount, address receiver, address maybeTokenAddress)
    {
        if (_data.length != 96) {
        ...
        (amount, receiver, maybeTokenAddress) = abi.decode(
            _data,
    ...
    function encodeAssetRouterBridgehubDepositData(
        ...
        return
            bytes.concat(
                NEW_ENCODING_VERSION,
                abi.encode(_assetId, _transferData)
            );
    }
    ...
    /// @param _assetData The asset data that has to be encoded.
    /// @return The encoded asset data.
    function encodeNTVAssetId(
        uint256 _chainId,
        bytes32 _assetData
    ) internal pure returns (bytes32) {
        return
            keccak256(
                abi.encode(_chainId, L2_NATIVE_TOKEN_VAULT_ADDR, _assetData)
            );
    }
    ...
    function encodeNTVAssetId(
        ...
        return
            keccak256(
                abi.encode(_chainId, L2_NATIVE_TOKEN_VAULT_ADDR, _tokenAddress)
            );
    }
    ...
    function encodeTxDataHash(
        ...
        if (_encodingVersion == LEGACY_ENCODING_VERSION) {
            address tokenAddress = INativeTokenVaultBase(_nativeTokenVault)
            ...
            bytes32 expectedAssetId = encodeNTVAssetId(
                block.chainid,
                tokenAddress
            );
            ...
            txDataHash = keccak256(
                abi.encode(_originalCaller, tokenAddress, depositAmount)
            );
            ...
            txDataHash = keccak256(
                bytes.concat(
                    _encodingVersion,
                    abi.encode(_originalCaller, _assetId, _transferData)
                )
            );
        } else {
        ...
        )
    {
        ...
        if (encodingVersion == LEGACY_ENCODING_VERSION) {
            (name, symbol, decimals) = abi.decode(
                _tokenData,
                (bytes, bytes, bytes)
            );
    ...
    function encodeTokenData(
        ...
        return
            bytes.concat(
                NEW_ENCODING_VERSION,
                abi.encode(_chainId, _name, _symbol, _decimals)
            );
    }
    ...
    function decodeBaseTokenFinalizeWithdrawalData(
        ...
        internal
        pure
        returns (bytes4 functionSignature, address l1Receiver, uint256 amount)
    {
        (uint32 functionSignatureUint, uint256 offset) = UnsafeBytes.readUint32(
            _l2ToL1message,
            0
        );
        ...
        )
    {
        ...
        require(
            _l2ToL1message.length >= 68,
            WrongMsgLength(68, _l2ToL1message.length)
        );
        // slither-disable-next-line unused-return
        (originChainId, offset) = UnsafeBytes.readUint256(
            _l2ToL1message,
            offset
        ); // originChainId, not used for L2->L1 txs
        ...
        )
    {
        ...
        functionSignature = bytes4(functionSignatureUint);
        bytes memory transferData = UnsafeBytes.readRemainingBytes(
            _l2ToL1message,
            offset
        );
